 ...
 */
export function createSession({ userId, expiresAt, ipAddress = null, userAgent = null }) {
 ...
 */
export function getActiveSessionsByUserId(userId) {
  const db = getDatabase();
  const stmt = db.prepare(
 ...
 */
export function updateSessionExpiration(sessionId, expiresAt) {
  const db = getDatabase();
 ...
 * @returns {Object|null} Updated session object or null if not found
 */
export function extendSession(sessionId, additionalMinutes) {
  const db = getDatabase();
  const stmt = db.prepare(

import { getDatabase, prepare } from '../db.js';
 ...
 */
export function getUserByEmailPublic(email) {
  const db = getDatabase();
  const stmt = db.prepare(
 ...
 */
export function setRecoveryKey(userId, recoveryPhraseHash) {
  const db = getDatabase();
 ...
 */
export function getRecoveryKey(userId) {
  const db = getDatabase();
 ...
 */
export function markRecoveryKeyUsed(userId) {
  const db = getDatabase();
  const stmt = db.prepare(
 ...
 */
export function deleteRecoveryKey(userId) {
  const db = getDatabase();
